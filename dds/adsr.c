...
int main(void){
  ...
  while(1){		       
	...
	if (noteClock > ATTACK_RATE*volume){ /* wait until time to increase next step */
	  if (volume < 31){
	    volume++;
	  }
	}
	...
	if ((noteClock - ATTACK_TIME) > (FULL_VOLUME-volume)*DECAY_RATE){
	...
	if ((noteClock - RELEASE_TIME) > (SUSTAIN_LEVEL-volume)*RELEASE_RATE){
	  if (volume > 0){
	    volume--;
	  }
	  else{
